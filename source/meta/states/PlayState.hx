...
class PlayState extends MusicBeatState
{
	...
	public var camOverlay:FlxCamera; // shit that should go above all else and not get affected by camHUD changes, but still below camOther (pause menu, etc) just kidding its under the hud now!!
	...
	override public function create()
	{
		...
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 240, 40, 400, "", 32);
		timeTxt.setFormat(Paths.font("retro.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timeTxt.scrollFactor.set();
		timeTxt.alpha = 0;
		timeTxt.borderSize = 2;
		timeTxt.antialiasing = false;
		timeTxt.visible = showTime;
		if(ClientPrefs.downScroll) timeTxt.y = FlxG.height - 47;
		...
		timeBarBG.x = timeTxt.x + 75;
		timeBarBG.y = timeTxt.y - 20;
		...
		timeBar = new FlxBar(timeBarBG.x + 10, timeBarBG.y + 32, LEFT_TO_RIGHT, 230, 23, this,
			'songPercent', 0, 1);
		...
		scoreTxt.setFormat(Paths.font("retro.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("retro.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		transition.setGraphicSize(camOther.width,camOther.height);
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Kill You':
				FlxG.save.data.anomalyspotted = true;
				FlxG.save.flush();
		...
		if(ret != Globals.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					Init.SwitchToPrimaryMenu(MainMenuState);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (!ClientPrefs.downScroll) 
			rating.y = camHUD.height * 0.78;
		else
			rating.y = 0.11 * camHUD.height;
